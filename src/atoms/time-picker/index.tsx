...
export function TimePicker({
  ...
  return (
    <div className="relative w-full" ref={ref}>
      ...
      />
      {open ? (
        <div className="absolute top-full right-0 z-20 mt-2 grid w-64 grid-cols-2 gap-4 rounded-lg border border-line bg-surface p-4 text-ink shadow-popover">
          <div>
            <p className="mb-2 text-2xs tracking-caps text-ink-dim uppercase">{locales.hours}</p>
            <div className="flex max-h-52 flex-col gap-1 overflow-y-auto">
              {HOURS.map((hour) => (
                <button
                  className={`rounded-sm px-3 py-2 text-left text-sm transition-colors duration-(--duration-fast) ease-out hover:bg-raised focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent ${hour === selectedHour ? "bg-accent text-on-accent hover:bg-accent-ink" : "text-ink-soft"}`}
                  key={hour}
          ...
          <div>
            ...
            <div className="flex max-h-52 flex-col gap-1 overflow-y-auto">
              {MINUTES.map((minute) => (
                <button
                  className={`rounded-sm px-3 py-2 text-left text-sm transition-colors duration-(--duration-fast) ease-out hover:bg-raised focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent ${minute === selectedMinute ? "bg-accent text-on-accent hover:bg-accent-ink" : "text-ink-soft"}`}
                  key={minute}
